...
def match(pattern: List[str], source: List[str]) -> List[str]:
    ...
    while len(pattern) > pind or len(source) > sind:
        ...
        # indicates the currentthing it pattern doesn't match the current thing in
        # source
        else:
            return(None)
    return result
...
if __name__ == "__main__":
   # print(match(["x", "z", "z"], ["x", "y", "z"]))
    assert match(["x", "y", "z"], ["x", "y", "z"]) == [], "test 1 failed" #1
    assert match(["x", "z", "z"], ["x", "y", "z"]) == None, "test 2 failed" #2
    assert match(["x", "y"], ["x", "y", "z"]) == None, "test 3 failed" #3
    assert match(["x", "y", "z", "z"], ["x", "y", "z"]) == None, "test 4 failed" #4
    print(match(["x", "_", "z"], ["x", "y", "z"]))
    assert match(["x", "_", "z"], ["x", "y", "z"]) == ["y"], "test 5 failed" #5
    assert match(["x", "_", "_"], ["x", "y", "z"]) == ["y", "z"], "test 6 failed" #6
    assert match(["%"], ["x", "y", "z"]) == ["x y z"], "test 7 failed" #7 
    assert match(["x", "%", "z"], ["x", "y", "z"]) == ["y"], "test 8 failed" #8
    assert match(["%", "z"], ["x", "y", "z"]) == ["x y"], "test 9 failed" #9
